...
namespace ISXApplication {
	...
	void Application::Start(const int argc, char** argv) {
		...
		if (argc == num_required_params) {
			...
			ISXManager::ConsoleManager::PrintMessage("To print chess field with specific values of height and width, "
				"please, pass two integers to the main class call");
		}
	}
	...
	void Application::SendFieldToPrinting() const {
	
		if (m_field) {
			ISXManager::ConsoleManager::DrawField(GetField());
		}
		else {
			ISXManager::ConsoleManager::PrintMessage("You entered wrong parameters for the field!");
		}
	}
